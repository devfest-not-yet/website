...
const MenuPlanner = () => {
    ...
    const getMealGradients = (mealType) => {
        const gradients = {
        ...
        return gradients[mealType] || 'from-gray-400/20 to-gray-500/20 border-gray-500/30';
    };

    const getAccentColor = (mealType) => {
        const colors = {
        ...
        <div className="space-y-8 pb-10">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                <div>
                    ...
                    <p className="text-muted-foreground font-medium mt-1">Curate healthy, balanced meals for all attendees.</p>
                </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
                <div className="lg:col-span-8 glass-card p-6 overflow-hidden">
                    <div className="flex items-center justify-between mb-8">
                        <div>
                            <h3 className="text-lg font-bold">Nutritional Balance Overview</h3>
                            <p className="text-xs text-muted-foreground font-medium">Aggregate data for today's active menu</p>
                        </div>
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                {nutritionData.map((item, i) => (
                                    <div key={i} className="p-4 rounded-2xl bg-muted/30 border border-border/40">
                                        ...
                                        <h4 className="text-xl font-bold" style={{ color: item.color }}>{item.value}%</h4>
                                    </div>
                                ))}
                            </div>
                    ...
                    <div className="glass-card p-6 bg-gradient-to-br from-indigo-600 to-blue-700 text-white border-none shadow-premium relative overflow-hidden group">
                        <div className="relative z-10 h-full flex flex-col justify-between">
                            <div>
                                ...
                                <p className="text-white/70 text-sm font-medium">Access 200+ approved kitchen recipes</p>
                            </div>
                            <div className="mt-10">
                                <div className="flex -space-x-3 mb-6">
                                    {[1, 2, 3, 4].map(i => (
                                        <div key={i} className="w-10 h-10 rounded-full border-2 border-indigo-600 bg-white/20 flex items-center justify-center font-bold text-xs backdrop-blur-sm">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                {menu.map((meal, index) => (
                    <motion.div
                        ...
                        className={`glass-card p-0 flex flex-col bg-gradient-to-br ${getMealGradients(meal.mealType)} border-none shadow-sm hover:shadow-premium transition-all duration-500 group overflow-hidden`}
                    >
                        <div className="p-6 pb-2">
                            <div className="flex items-center justify-between mb-2">
                                <span className="text-3xl filter grayscale group-hover:grayscale-0 transition-all duration-500">{getMealIcon(meal.mealType)}</span>
                                <span className={`text-[10px] uppercase tracking-widest font-bold ${getAccentColor(meal.mealType)}`}>
                                    {meal.mealType}
                                </span>
                            </div>
                            <h3 className="text-xl font-bold text-foreground mb-2 group-hover:text-primary transition-colors">{meal.mealName}</h3>
                            <p className="text-xs text-muted-foreground font-medium leading-relaxed mb-4">{meal.description}</p>
                        </div>
                        ...
                        <div className="px-6 py-4 bg-white/40 dark:bg-background/40 backdrop-blur-sm border-t border-white/40 dark:border-white/5 flex-grow">
                            <h4 className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground mb-4 opacity-70">Nutrition Profile</h4>
                            <div className="grid grid-cols-2 gap-y-4 gap-x-2">
                                <div className="flex items-center gap-2">
                                    <div className={`p-1.5 rounded-lg bg-background/50 border border-border/20 ${getAccentColor(meal.mealType)}`}>
                                        <Zap size={12} />
                                    </div>
                                    <span className="text-xs font-bold">{meal.nutrition.calories} kcal</span>
                                </div>
                                <div className="flex items-center gap-2">
                                    <div className={`p-1.5 rounded-lg bg-background/50 border border-border/20 ${getAccentColor(meal.mealType)}`}>
                                        <Target size={12} />
                                    </div>
                                    <span className="text-xs font-bold">{meal.nutrition.protein}g Protein</span>
                                </div>
                            </div>
                        </div>

                        <div className="p-6 pt-4 bg-white/20 dark:bg-background/20 backdrop-blur-md">
                            <h4 className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground mb-3 opacity-70">Main Ingredients</h4>
                            <div className="flex flex-wrap gap-2">
                                {meal.ingredients.slice(0, 4).map((ing, i) => (
                                    <span key={i} className="px-2.5 py-1 rounded-lg bg-background/50 border border-border/20 text-[10px] font-bold text-muted-foreground shadow-sm">
                                        {ing}
                                    </span>
                                ))}
