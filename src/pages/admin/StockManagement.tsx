...
const StockManagement: React.FC = () => {
  ...
  const inventory: StockItem[] = (stockResponse?.success && stockResponse.data) ? stockResponse.data : [];
  ...
  const columns = [
    {
      ...
      render: (_value, row) => (
        <div className="flex items-center gap-3">
          ...
          <span className="font-bold">{row.ingredient.name}</span>
        </div>
      ),
    },
    {
      ...
      render: (value, row) => (
        <div className="flex flex-col">
          <span
            className={`font-bold ${value <= 50 ? "text-destructive" : ""}`}
          >
            {value} {row.unit || "kg"}
          </span>
          <div className="w-20 h-1 bg-muted rounded-full mt-1 overflow-hidden">
            <div
              className={`h-full ${value <= 20
                ? "bg-destructive"
                : value <= 50
                  ? "bg-warning"
                  : "bg-success"
  ...
  const lowStockItems = inventory.filter(
    (item) => item.available_quantity <= 50
  );
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <p className="text-muted-foreground font-medium">
            Track and manage your kitchen inventory in real-time.
          </p>
        </div>
        <div className="flex p-1 bg-muted/50 rounded-xl w-fit">
          <button
            onClick={() => setActiveTab("inventory")}
            className={`px-4 py-2 rounded-lg text-sm font-bold transition-all ${activeTab === "inventory"
          ...
          <button
            ...
            className={`px-4 py-2 rounded-lg text-sm font-bold transition-all ${activeTab === "analytics"
              ? "bg-background shadow-sm text-primary"
              : "text-muted-foreground hover:text-foreground"
              }`}
          >
                      ...
                      <div className="relative z-10 flex items-start gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-sm font-medium text-destructive/80 mt-1">
                            {lowStockItems.length} items are below critical
                  ...
                  <div className="h-20 w-full relative z-10">
                    <ResponsiveContainer width="100%" height="100%">
                      <AreaChart
                        data={inventory
                          .slice(0, 5)
                          .map((item) => ({ v: item.available_quantity }))}
                      >
                        <Area
                          type="monotone"
                          dataKey="v"
                ...
                <div className="glass-card p-6">
                  ...
                  <div className="space-y-4">
                    {[
                      {
                        label: "Dry Storage",
                        val: inventory.length > 0 ? 45 : 0,
                        color: "bg-indigo-500",
                      },
                      {
                        label: "Cold Storage",
                        val: inventory.length > 0 ? 30 : 0,
                        color: "bg-sky-500",
                      },
                      {
                        label: "Frozen Section",
                        val: inventory.length > 0 ? 15 : 0,
